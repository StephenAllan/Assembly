...
block_area:
	...
	lw	$t0, 4($a0)			# load width
	lw	$t1, 8($a0)			# load height
...
block_perimeter:
	...
	add 	$t0, $t0, $t0			# width * 2
	add 	$t1, $t1, $t1 			# height * 2

	add 	$v0, $t0, $t1			# P = W*2 + H*2
...
# Note that this does not involve any polymorphic functions.
	...
	li	$v0, PRINT_INT	# Print_int system code
	lw 	$a0, 4($s0)	# Load width
	syscall 		# Print width
...
# the address of the figure object, and shouldn't be changed.
	...
	move	$a0, $s0	
	lw	$t0, 0($a0)	# Load address of the virtual function table
	lw	$t0, 0($t0)	# Load address of the area function

	jalr	$t0		# Call proper area function
	move	$a0, $v0	# Copy area
...
# contains the address of the figure object, and shouldn't be changed.
	...
	move	$a0, $s0	
	lw	$t0, 0($a0)	# Load address of the virtual function table
	lw	$t0, 4($t0)	# Load address of the perimeter function

	jalr	$t0		# Call proper perimeter function
	move	$a0, $v0	# Copy perimeter
